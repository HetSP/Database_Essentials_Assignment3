...
-- Fetch user's timeline(All tweets from users whom I am following with tweet content and user name who has tweeted it)(change the f.follower_userid as need)
select u.name,t.content from tweet as t inner join follow as f on t.userid=f.following_userid inner join user as u on f.following_userid=u.userid where f.follower_userid="1";